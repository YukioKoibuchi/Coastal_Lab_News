...
# （2026-08-05追加）。CORE_KEYWORDSに載っているだけでは足りず、ここにも入っている
# 必要がある。analyze_feedback.pyで「判定5件以上・却下率が高いフラグなし」を確認できた
# キーワードだけを手動でここに追加すること（2026-08-05時点でのdecisions.csv 88件の
# 分析結果に基づく）。ここに無いキーワードだけで一致した記事は、generate_site_news.py
# では自動公開されず、これまで通りreview_server.pyでの人手レビューに回る。
# 対象外（判定件数不足）: ヘッドランド、海難事故、高潮、高波、海岸線、養浜、サンゴ礁、
#   沿岸、海浜、津波（追加したばかりで判定データ無し）
# 対象外（判定件数はあるが採用率が低い・要観察）: マングローブ（33.3%、6件）
TRUSTED_KEYWORDS = [
